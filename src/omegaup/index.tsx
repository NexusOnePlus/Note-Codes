...
const Hk = () => {
  ...
  return (
    <div className="bg-gradient-to-t from-indigo-950 to-black min-h-screen gap-6 text-white p-10">
      <div>
      ...
      <div className="grid place-items-center grid-flow-row lg:grid-cols-4 h-full gap-3 lg:gap-7 text-center mt-10">
        {hacks.map((hack: any) => {
          return (
            <Link key={hack.id} to={`/omegaup/${hack.id}`}>
              <div
                key={hack.id}
                className="bg-blue-900/20  border-4 border-cyan-300 h-[100px]  w-[250px] lg:h-[150px] p-5 content-center rounded-2xl text-[20px] lg:text-[30px] font-bold"
              >
